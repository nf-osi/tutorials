...
- In Cavatica studio, here are the files and their meta.
</div>
...
This introduces two different analysis options that you can compare with the Cavatica workflow.
</div>
...
- Saw that data was "pre-loaded" for analysis, no effort needed
- Learned exploratory analysis features
- Data not guaranteed to be most up-to-date
- Saw limitations for more flexible computational analysis
...
**SETUP NOTES IN COLAB**
1. Make sure logged into your Google account.
2. At the notebook, fork the notebook to get your own copy.
3. You can run the analysis with *your* Synapse credentials for the *second* exercise. 
(the first exercise requires submitting a Data Use Statement, which we'll show how to do, but you won't get approved immediately should you choose to do this.) 
4. Click on key icon at left. It'll open with an option for `Add new secret`.
5. Go to your Synapse profile (switch tab) -> Personal Access Token -> generate one and copy it.
6. Add a secret called `SYNAPSE_AUTH_TOKEN` and paste this in. Turn on the toggle of "Notebook access".

</div>

## Debrief X

- Encountered variant of data access flow with add'l governance requirements, familiarized with steps needed
- Learned basics of how to connect/load the data in this particular analysis environment, some effort needed
- Learned features and what analysis looks like in this particular analysis environment

<div class="notes">
Through the workshop exercise just now, you saw:
- An example of going through Conditions of Use to access data.
...
- Learned basics of how to get data from Synapse
- Better understanding of analysis possibilities through hands-on comparison and interaction
- Better ability to assess for yourself what is good for your use case on the usability vs flexibility spectrum
- Where to start computationally to take advantage of category 3 analysis environments  
- Basics of collaborative and reproducible analysis
